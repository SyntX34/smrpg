...
{
    if(g_SpeedMethod == SM_MaxSpeed && g_hGetSpeed != null)
        g_iClientHookId[client] = DHookEntity(g_hGetSpeed, true, client);
}

public void OnClientDisconnect_Post(int client)
{
    if(g_iClientHookId[client] != 0)
        DHookRemoveHookID(g_iClientHookId[client]);
...
public void ConVar_OnSpeedMethodChanged(ConVar convar, const char[] oldValue, const char[] newValue)
{
    ...
    g_SpeedMethod = view_as<SpeedMethod>(StringToInt(newValue));
